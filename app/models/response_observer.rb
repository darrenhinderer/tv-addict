class ResponseObserver < ActiveRecord::Observer
  ...
  def after_create(response)
    response.facebook_user.set_profile_info("TV Addict", info_fields(response.facebook_user))
  end
  ...
  def info_fields(user)
    [{:field => "What I'm Watching", :items => watching_list(Response.alphabetized_responses(user.id))},
     {:field => "What I've Watched", :items => watched_list(Response.alphabetized_responses(user.id))}]
  end
